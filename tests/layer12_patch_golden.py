#!/usr/bin/env python3
"""
Golden for Layer 12: Conv 3x3, 64->64, stride 1 (576 MACs per pixel)
Loads layer11 output + layer12 weights/bias, computes output pixel (0,0) for channels 0..N-1
using Phase 3 primitives, and writes vectors for RTL comparison.

Run from project root after run_image_to_detection.py --layers 13
Output: image_sim_out/layer12_patch_*.hex, layer12_patch_golden.json
"""
NUM_CHANNELS = 4
...
def main():
    ...
    if sim_out is None:
        print("Run 'run_image_to_detection.py --layers 13' first")
        return 1

    inp = np.load(sim_out / "layer11_output_ref.npy")   # (64, 52, 52)
    w = np.load(sim_out / "layer12_weights.npy")          # (64, 64, 3, 3)
    b = np.load(sim_out / "layer12_bias.npy")             # (64,)

    print(f"Layer12 patch golden (one pixel (0,0), {NUM_CHANNELS} channels)")
    ...
    with open(hex_dir / "layer12_patch_a.hex", "w") as f:
    ...
    for ch in range(NUM_CHANNELS):
        ...
        with open(hex_dir / f"layer12_patch_w{ch}.hex", "w") as f:
    ...
    with open(hex_dir / "layer12_patch_bias.hex", "w") as f:
        f.writelines(bias_lines)
    with open(hex_dir / "layer12_patch_expected.hex", "w") as f:
    ...
    with open(sim_out / "layer12_patch_golden.json", "w") as f:
        json.dump(golden, f, indent=2)

    print(f"  Written: layer12_patch_golden.json, layer12_patch_*.hex")
    return 0
