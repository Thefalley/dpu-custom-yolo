#!/usr/bin/env python3
"""
DPU Top 36-layer check: Python golden -> RTL sim -> compare.
...
Usage:
  python run_dpu_top_36layer_check.py
  python run_dpu_top_36layer_check.py --python-only
"""
...
def run_rtl():
    ...
    r = subprocess.run(cmd, cwd=PROJECT_ROOT, capture_output=True, text=True,
                       timeout=7200, env=env)  # 2hr timeout for large sim
    out = (r.stdout or "") + (r.stderr or "")
    return "ALL 36 LAYERS PASS" in out, out


def main():
    import argparse
    ap = argparse.ArgumentParser(description="DPU Top 36-layer check")
    ...
    print("=" * 60)
    print(f"DPU Top 36-Layer Check (H0=32, W0=32) [{mode}]")
    print("=" * 60)
